...
<head>
  ...
  <title>Registrasi User - Titipangan</title>
  ...
  <style>
    ...
    .input-field { width:100%; border-radius:10px; border:1px solid rgba(1,129,54,.2); background:rgba(255,255,255,.9); padding:12px 16px; outline:none; font-size:14px; }
    .input-field:focus { border-color:rgba(1,129,54,.5); box-shadow:0 0 0 3px rgba(1,129,54,.08); }
    .auth-panel-left, .auth-panel-right, .auth-form-card { will-change: transform, opacity; }
    .from-login .auth-panel-left { animation: slideInLeft .45s ease both; }
    .from-login .auth-panel-right { animation: slideInRight .45s ease both; }
    .from-login .auth-form-card { animation: popIn .4s ease .05s both; }
...
<body class="min-h-screen font-sans text-brand-secondaryDark bg-brand-bg">
  <main class="min-h-screen grid md:grid-cols-2">
    <section class="auth-panel-left relative min-h-screen text-white overflow-hidden" style="background:linear-gradient(165deg, #01361A 0%, #005B21 55%, #007E34 100%);">
      ...
      <div class="relative z-10 h-full p-10 lg:p-14 flex flex-col justify-between">
        ...
        <div class="space-y-4 max-w-lg">
          ...
          <p class="text-base leading-relaxed text-white/80">Daftarkan akun untuk mengakses dashboard, memantau jatah pengambilan mingguan, dan mengikuti regulasi foodbank dengan transparan.</p>
        </div>
        <p class="text-xs text-white/55">Aman, cepat, dan berkelanjutan.</p>
      </div>
    </section>

    <section class="auth-panel-right relative min-h-screen flex items-center justify-center bg-brand-bg overflow-hidden">
      ...
      <div class="auth-form-card relative z-10 w-full max-w-md rounded-2xl mx-4 md:mx-0 border border-brand-secondaryLight/60 bg-white shadow-xl p-8 space-y-6">
        <div class="space-y-2">
          <p class="text-[10px] font-semibold tracking-[0.2em] uppercase text-brand-secondaryDark/45">New Member</p>
          <h2 class="font-display text-3xl font-semibold text-brand-forest">Registrasi User</h2>
          <p class="text-sm text-brand-secondaryDark/60">Daftar dengan nomor HP, lalu verifikasi OTP WhatsApp.</p>
        </div>

        <form method="POST" action="/user/register/send-otp" class="space-y-4">
          ...
          <button class="w-full rounded-xl bg-brand-btn text-white font-semibold px-4 py-3 hover:bg-red-800 transition-colors">Kirim OTP Registrasi</button>
        </form>

        <p class="text-sm text-brand-secondaryDark text-center">Sudah punya akun? <a href="/user/login" data-auth-link="login" class="font-semibold text-brand-heading hover:underline">Login di sini</a></p>
  ...
  <script>
    document.querySelectorAll('[data-auth-link]').forEach((el) => {
      el.addEventListener('click', () => sessionStorage.setItem('authTransitionFrom', 'register'));
    });
    const from = sessionStorage.getItem('authTransitionFrom');
    if (from === 'login') document.body.classList.add('from-login');
